...
def vasp_parser_with_test(calc_with_retrieved, request):
    ...
    parser.add_parser_definition(
        ...
        {'parser_class': ExampleFileParser, 'is_critical': False, 'mode': 'r'},
    )
...
def test_add_parser_quantity(vasp_parser_without_parsing):
    ...
    parser.add_parsable_quantity(
        'quantity_with_alternatives',
        {'inputs': [], 'prerequisites': [], 'name': '_scheduler-stderr.txt'},
    )
    parser.parse(retrieved_temporary_folder=path)
...
def test_quantity_uniqeness(vasp_parser_with_test):
    ...
    parser.add_parser_definition(
        'another_test_parser',
        {'parser_class': ExampleFileParser2, 'is_critical': False, 'mode': 'r'},
    )
...
@pytest.mark.xfail(
    ...
    reason='Element X only present in Aiida >= 1.x',
)
def test_parse_poscar_silly_read(fresh_aiida_env, data_path):
    """
...
def test_parse_kpoints(vasp_kpoints, data_path):
    """
...
def test_structure(request, calc_with_retrieved):
    ...
    np.testing.assert_allclose(
        np.round(structure_vasprun.cell, 7),
...
def test_misc(request, calc_with_retrieved):
    ...
    # Path(request.fspath) will be replaced by request.node.path from pytest v7.
    file_path = str(pathlib.Path(request.fspath).parent / '../test_data/disp_details')
    node = calc_with_retrieved(file_path, settings_dict)
    parser_cls = ParserFactory('vasp.vasp')
...
def test_custom_outputs(request, calc_with_retrieved):
    ...
    parser_settings = {
        ...
        'add_fermi_level': {'type': 'core.float'},
    }
...
def test_notification_composer(vasp_parser_without_parsing):
    ...
    composer = NotificationComposer(
        ...
        {},
        {
            'parameters': get_data_class('core.dict')(dict={'nelect': 10}),
        },
        parser.exit_codes,
...
def test_critical_object_missing(calc_with_retrieved, request):
    ...
    parser.add_parser_definition(
        ...
        {'parser_class': ExampleFileParser, 'is_critical': True, 'mode': 'r'},
    )
    parser.add_parsable_quantity(
        'quantity_with_alternatives',
        {'inputs': [], 'prerequisites': [], 'file_name': '_scheduler-stderr.txt'},
    )
