...
def test_relax_wc(fresh_aiida_env, upload_potcar, mock_vasp, data_path, potcar_family_name, potcar_mapping):
...
def setup_vasp_relax_workchain(structure, incar, nkpts, potcar_family_name, potcar_mapping, code=None):
    """
...
def test_vasp_wc_ionic_magmom_carry(
    ...
    incar['lorbit'] = 10
    incar['nupdown'] = 2
    inputs = setup_vasp_relax_workchain(si_structure(), incar, 8, potcar_family_name, potcar_mapping)
    inputs.verbose = get_data_node('core.bool', True)
    ...
    assert (
        called_nodes[1].inputs.site_magnetization['site_magnetization']['sphere']['x']['site_moment']['1']['tot']
        == 0.643
    )
