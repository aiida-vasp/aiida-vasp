...
after a [VASP] calculation are finished and this tutorial touches on this to give you
a short introduction on how this is done and where to find more information about this.
Let us continue with the [VASP] tutorials. In particular, let us now extract
the [density of states for FCC Si] as done in the second tutorial. We will from here
and in the rest of the tutorials not run [VASP] in the regular manner and leave those
exercise to the reader. Please do so to get a feel for how [AiiDA-VASP] makes
...
1. Let us first create a simple call script that uses the {ref}`vasp_workchain`:
...
4. Check the content of the `VaspWorkChain`:
   ...
   With this we conclude the initial tutorials that follow the [VASP] tutorials closely
