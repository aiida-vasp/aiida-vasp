(tutorial-fcc-si-step1)=

# 1. Introduction

Here we will introduce the plugin. We will now follow the [FCC Si] example in the
[VASP tutorials] tutorial. We will
obtain the same data, but using different strategies so that you as a user will
be familiar with what is going on with the plugin. If you are a familiar [VASP] user what we will
go through should be well known. The [FCC Si] tutorial is all about calculating
...
[AiiDA-VASP] and then continue to execute the first static (fixed volume) calculation
with [AiiDA-VASP] in order to get to know how the submission, monitoring and inspection
process works. In the {ref}`next tutorial<tutorial_fcc_si_step2>` will start to perform
calculations at different volumes.

Before starting, we would like you to get a bit familiar with the concepts of [AiiDA], so please
have a look at [AiiDA concepts] before continuing.

01. First complete the [FCC Si] tutorial without using [AiiDA] or the [AiiDA-VASP] plugin.

02. Enable your [AiiDA] virtual environment where [AiiDA-VASP] is installed.

03. Make sure your [AiiDA] daemon runs:

    ```
    $ verdi daemon start
    ```

04. We will now following the lines of the [FCC Si] tutorial, but using [AiiDA-VASP]. In the process
    we will be touching different strategies to you get a feel for how you can structurize a simple workflow.
    Let us fetch the [AiiDA-VASP] run file for this example:

    ```
    $ wget https://github.com/aiida-vasp/aiida-vasp/raw/master/tutorials/run_fcc_si_one_volume.py
    ```

05. Inspect the file, which has the following content:

    ```{literalinclude} ../../../tutorials/run_fcc_si_one_volume.py
    ```

06. Change the `CODE_STRING`  based on the code and computer you have stored. This can be
    inspected with `verdi code list`.

07. Change the following to comply with requirements of your cluster or your project:
...
08. Save and execute the resulting run script by issuing:

    ```
    $ python run_fcc_si_one_volume.py
    ```

09. Check its progress with:
    ...
    process in [AiiDA] which launches the actual VASP calculation. This is presently in the `Waiting for transport task: upload`
...
10. After a while, we execute `verdi process list -a` and get:
    ...
    execution. Please consult the documentation of the [AiiDA exit codes] for more details. [AiiDA]
...
11. Let us have a look at what happened during the execution of the workflow. We typically inspect the topmost,
...
12. Let us have a look at what is stored on the {ref}`vasp_workchain` with `PK` of `883`.
    ...
    :::{note}
    Most things in [AiiDA] that are stored are considered a `node` and we will continue to use this terminology and
    ...
    [AiiDA] as explained in [AiiDA concepts].
    :::
...
[aiida concepts]: https://aiida.readthedocs.io/projects/aiida-core/en/latest/topics/provenance/concepts.html
[aiida exit codes]: https://aiida.readthedocs.io/projects/aiida-core/en/latest/topics/processes/usage.html#exit-codes
[aiida job resources]: https://aiida.readthedocs.io/projects/aiida-core/en/latest/topics/schedulers.html?highlight=resources#job-resources
[aiida-vasp]: https://github.com/aiida-vasp/aiida-vasp
[fcc si]: https://www.vasp.at/wiki/index.php/Fcc_Si
