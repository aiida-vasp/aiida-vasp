...
from .parser_new import MissingFileError, NotificationComposer, QuantityMissingError, VaspParser
...
class NebParser(VaspParser):
    ...
    def _compose_structure(self, quantities_each):
        ...
        for index in self.neb_indices:
            ...
            node.set_cell(data[index]['unitcell'])
            for site in data[index]['sites']:
