...
def load_structure_from_file(structure_path: str | Path) -> orm.StructureData:
    """Load a structure from various file formats."""
    structure_path = Path(structure_path)
    if not structure_path.exists():
        raise click.ClickException(f'Structure file not found: {structure_path}')
    ...
    if extension in ['.cif']:
        ...
        cif_node, _ = cif_data.get_or_create(str(structure_path))
...
def handle_calculation_submission(upd: VaspBuilderUpdater, run_directly: bool, group: str):
    ...
    if group:
        try:
            ...
            click.echo(f"{action} group '{group}' and added calculation")
        except Exception as e:
