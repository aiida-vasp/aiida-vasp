...
def compare_structures(structure_a, structure_b):
    ...
    site_vectors = [delta.absolute.pos[i, :]
                    for i in range(delta.absolute.pos.shape[0])]
    ...
    delta.absolute.cell_angles = np.absolute(
        cell_angles_a - np.array(structure_b.cell_angles))
